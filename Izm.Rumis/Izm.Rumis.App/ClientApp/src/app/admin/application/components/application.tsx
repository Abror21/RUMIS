...
const Application = ({ applicationId, initialData, initialSocialStatus }: ApplicationProps) => {
  ...
  const [socialStatus, setSocialStatus] = useState<boolean | undefined>(initialSocialStatus);
  ...
  return (
    <div>
      <Spin spinning={isLoading}>
        {typedData &&
          <div className="flex gap-6 max-w-[1392px]">
            <div className="w-3/5 bg-white rounded-lg p-6">
              ...
              <div className="grid grid-cols-5 p-3">
                ...
                <div className="col-span-3">{`${typedData?.resourceTargetPersonClassGrade}${typedData?.resourceTargetPersonClassParallel}`}</div>
              </div>
              <div className="grid grid-cols-5 p-3">
                <div className="col-span-2">
                </div>
              ...
              <div className="grid grid-cols-5 gap-6 p-3 ps-0">
                ...
                <div className="col-span-3">{typedData?.resourceTargetPersonEducationalStatus?.value ?? typedData?.resourceTargetPersonWorkStatus?.value}</div>
              </div>
              <div className="grid grid-cols-5 p-3 border-b">
                <div className="col-span-2">
                </div>
                <div className="col-span-3">{typedData?.resourceTargetPersonEducationalSubStatus?.value}</div>
              </div>
                  ...
                  <ul className="list-disc">
                    {
                      typedData?.applicationSocialStatus?.map(status => (
                        <li key={status.id} className="text-gray-400">
                          ...
                          <span className="text-black">{status?.socialStatus?.value}</span>
                        </li>
                      ))
                    }
                  </ul>
            ...
            <div className="w-2/5">
              ...
              >
                <Form form={form} layout="vertical" onFinish={handleContactSubmit}>
                  ...
                  <Title level={5}>Vai norādīt Jūs kā kontaktpersonu visos {form.getFieldValue('firstName')} {form.getFieldValue('lastName')} pieteikumos?</Title>
