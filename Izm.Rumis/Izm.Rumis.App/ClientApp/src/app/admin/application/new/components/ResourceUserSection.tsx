  ...
  const isGetDataButtonDisabled = useMemo(() => {
    ...
    const {
      ...
      const { refetch: getData } = useQueryApiClient({
        request: {
            url: idintifyRequest().url,
          data: {
            privatePersonalIdentifier: idintifyRequest().privatePersonalIdentifier
          },
          method: 'GET',
          disableOnMount: true
        },
        onSuccess: (response: any[]) => {
          if (hasMatchingEducationalInstitutionId(response, sessionData?.user?.educationalInstitutionId)) {
            ...
            if (response.length > 0) {
              ...
              handleError({error: 'Pēc šī personas koda nav atbilstošu datu'})
            }
          }
},
        onFinally: () => {
                    ...
                    <Form.Item
                        ...
                        rules={[
                          { required: true, message:"Lūdzu ievadiet izglītojamā personas kodu." },
                          {
                            ...
                            message: 'Atļauts ievadīt tikai 11 ciparus',
                          }
                        ]}
                        className='w-full sm:w-[200px]'
                    >
