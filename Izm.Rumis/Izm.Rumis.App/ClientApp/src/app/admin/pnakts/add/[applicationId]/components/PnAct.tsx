...
import { RESOURCE_STATUS_AVAILABLE } from "@/app/admin/application/new/components/applicantConstants"
...
const PnAct = ({data, applicationId, isAddForm, pnact}: PnActProps) => {
    ...
    const selectedResource: Resource | null = useMemo(() => {
        ...
        return filteredResourceOptions.find(opt => opt.id === resource) as Resource
    }, [resource])
    ...
    const {appendData: refetchResources, isLoading: isResourceLoading} = useQueryApiClient({
        ...
        onSuccess(response) {
            ...
            if (isAddForm && data?.resourceSubType.id) {
                const filteredValues = response.items.filter((type: any) => type.resourceSubType.id === data?.resourceSubType.id)
                setFilteredResourceOptions(filteredValues)
            }
        },
    });
    ...
    const onSearch = (searchText: string) => {
        ...
        searchTimer = setTimeout(() => {
            ...
            refetchResources({
                ...
                resourceSubTypeIds: resourceSubTypeIds ?? null
            })
        }, 500)
    }
            ...
            >
                {data &&
                    <div className="w-1/3">
                        ...
                        >
                            <DatePicker format={datePickerFormat} placeholder="Termiņš"/>
                        </Form.Item>
                        {isAddForm ?
