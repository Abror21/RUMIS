...
{
    /// <inheritdoc />
    public partial class UpdateOpenDataView : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                ...
                "   ResourceType.Value AS 'Resursa veids', " +
                ...
                "FROM Applications " +
                "INNER JOIN EducationalInstitutions ON Applications.EducationalInstitutionId = EducationalInstitutions.Id " +
                "INNER JOIN Supervisors ON EducationalInstitutions.SupervisorId = Supervisors.Id " +
                "INNER JOIN Classifiers AS ResourceSubType ON Applications.ResourceSubTypeId = ResourceSubType.Id " +
                "INNER JOIN Classifiers AS ResourceType ON ResourceType.`Type` = 'resource_type' AND ResourceType.Code = JSON_UNQUOTE(JSON_EXTRACT(ResourceSubType.Payload, '$.resource_type')) " +
                "INNER JOIN Classifiers AS ApplicationStatus ON Applications.ApplicationStatusId = ApplicationStatus.Id " +
                "GROUP BY EducationalInstitutions.Name, Supervisors.Name, ResourceType.Value, ResourceSubType.Value, DATE(Applications.ApplicationDate) " +
                "ORDER BY DATE(Applications.ApplicationDate) ASC, EducationalInstitutions.Name ASC, ResourceSubType.Value ASC;");
        }
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(
                ...
                "       WHERE ResourceType.`Type` = 'resource_type' AND ResourceType.Code = JSON_UNQUOTE(JSON_EXTRACT(ResourceSubType.Payload, '$.resource_type')) " +
                "       LIMIT 1" +
                "   ) AS 'Resursa veids', " +
                "   ResourceSubType.Value AS 'Resursa paveids', " +
                "   DATE(Applications.ApplicationDate) AS 'Datums', " +
